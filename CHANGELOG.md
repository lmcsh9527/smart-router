...
## [v5.4] 2026-08-21 修复图片路由：长上下文因残留截图全走识图模型

### 修复
- `has_image_part` 原来扫描整条消息历史，DSH 带完整历史导致只要曾经贴过图，
  后续所有纯文字回合都被判为「含图」→ 全走识图模型（慢，48s/23s）
- 改为 `has_new_image`：只查【当前轮】最新消息是否含图；历史旧图不再影响路由
- 文字路由新增 `strip_image_parts`：剥掉历史消息里的图片段，
  防止文本模型收到 image_url 报错

### 实测
- 历史含图 + 当前轮纯文字 → 走 flash (c0)，0.48s ✅
- 当前轮带图 → 走 sensenova 识图模型 ✅
- 单测 has_new_image / strip_image_parts 全过 ✅

## [v5.5] 2026-08-21 缓存价配置 + 缓存节省统计卡

### 变更
- 机缘 flash/pro 模型配置 `price_cached`（flash=0.1/pro=0.3，业界缓存价 1/10）
- 统计卡新增「缓存已节省」大数字（绿色），与「总费用」并排对比
- 未配缓存价时，缓存 Token 有值但节省=¥0 → 统计卡提示「⚠️ 缓存模型未配缓存价，费用按全价计」
- 配好缓存价后，每次缓存命中自动计算节省金额（cached/1M × (price_in - price_cached)）

### 实测
- 配好 price_cached 后，今日 5.4M 缓存命中 → 累计节省 ¥3.70 ✅
- 后台页 `http://localhost:20130/admin` 统计卡可见「缓存命中 Token」「缓存已节省」两列
